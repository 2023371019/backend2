...
app.get("/api/reporte-ventas", async (req, res) => {
  try {
    ...
    // Obtener cantidad total de productos vendidos por cada usuario
const [productosPorUsuario] = await db.promise().query(`
  SELECT u.id_usuario, CONCAT(u.nombre, ' ', u.apellido) AS nombre_usuario, 
         COALESCE(SUM(dv.cantidad), 0) AS productos_vendidos
  FROM checkme_usuarios u
  JOIN checkme_ventas v ON u.id_usuario = v.id_usuario
  JOIN checkme_detalle_ventas dv ON v.id_venta = dv.id_venta
  GROUP BY u.id_usuario
`);


    // Mapear por id_usuario para acceso rápido
    const productosVendidosMap = {};
    productosPorUsuario.forEach((u) => {
      productosVendidosMap[u.id_usuario] = {
        nombre_usuario: u.nombre_usuario,
        productos_vendidos: u.productos_vendidos,
      };
    });

    // Recorrer cada mes y obtener sus detalles desde la vista
    for (let mesData of result[0]) {
      const [detalles] = await db.promise().query(
        `SELECT id_venta, fecha, cliente, total, id_usuario 
         FROM VistaReporteVentasPorMes
         WHERE mes = ?`,
        [mesData.mes]
      );

      // Agregar info adicional por usuario a cada venta
      const detalleConUsuario = detalles.map((venta) => {
        const info = productosVendidosMap[venta.id_usuario] || {
        ...
        return {
          ...
          productos_vendidos: info.productos_vendidos,
        };
      });

      reporte.push({
        ...mesData,
        ventas_detalle: detalleConUsuario,
      });
    }
